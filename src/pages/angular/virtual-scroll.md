---
title: 'Virtual Scroll in Angular'
---

In previous versions of Ionic, there had been a `virtual-scroll` directive to help with list virtualization. But as of recent, Angular provides it's own solution with the `@angular/cdk` package.

## Install

To install the CDK Scroller, first install `@angular/cdk`:
...
The CDK Virtual Scroller can be added to a component by add the `cdk-virtual-scroll-viewport` to a component's template.
...
Here, `items` is an array, but it can be an array, `Observable<Array>`, or `DataSource`. `DataSource` is an abstract class that can provide the data needed as well as utility methods. For more details, checkout the [CDK Virtual Scroller docs](https://material.angular.io/cdk/scrolling/overview).
...
This only covers a small part of what the CDK Virtual Scroller is capable of. For more details, please see the [docs](https://material.angular.io/cdk/scrolling/overview) on Angular CDK's website.
